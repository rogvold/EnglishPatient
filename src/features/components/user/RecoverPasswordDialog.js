...
var RecoverPasswordDialog = React.createClass({
    ...
    submit: function(){
        ...
        if (LoginMixin.validateEmail(email) == false){
            this.setState({
        ...
        LoginMixin.recoverPassword(email, function(){
            self.setState({
                loading: false,
                message: 'На введенный почтовый адрес отправлена ссылка для восстановления пароля. ' +
                'Проверьте почту и кликните по ссылке.'
            });
    ...
    render: function () {
        var submitEnabled = LoginMixin.validateEmail(this.state.email);
                    ...
                    <div style={this.componentStyle.panelPlaceholder}>
                        ...
                        <div style={this.componentStyle.submitButtonPlaceholder} onClick={this.submit} >
                            <button disabled={!submitEnabled} className={'ui button patientPrimary'} >
                                <i className={'icon wizard'}></i> восстановить пароль
                            </button>
                        </div>
