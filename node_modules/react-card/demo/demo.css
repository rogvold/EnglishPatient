...
.card, .card .card-front, .card .card-back {
  width: 500px;
  height: 500px;
...
.card.card-flipped .card-transform {
  -webkit-transform: rotateY(180deg);
          transform: rotateY(180deg);
...
.index-card .card-back {
  background-image:
      -webkit-linear-gradient(270deg, white 30px, #F0A4A4 30px, #F0A4A4 32px, transparent 1px), -webkit-repeating-linear-gradient(270deg, transparent, transparent 25px, #DDD 1px, #DDD 26px);
  background-image:
...
html::before{
  ...
  content: "\203a  postcss-import: /Users/ssorense/Documents/workspace/react-card/demo/demo.postcss:1:1: It looks like you didn\'t end correctly your @import statement. Some children nodes are attached to it";
